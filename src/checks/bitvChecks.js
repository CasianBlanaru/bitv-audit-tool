...
const BITV_CHECKS = {
  ...
  '3.3.4': {
    ...
    check: async (page) => {
      ...
      const results = await page.evaluate(() => {
        const errors = [];
        const forms = document.querySelectorAll('form[action*="submit"], form[action*="payment"], form[action*="order"]');
        for (const form of forms) {
          const submitButton = form.querySelector('button[type="submit"], input[type="submit"]');
          const confirmStep = form.querySelector('[aria-label*="bestätigen"], [textContent*="Bestätigen"], [textContent*="Überprüfen"]') ||
  ...
  '1.4.11': {
    ...
    check: async (page) => {
      ...
      const results = await page.evaluate(() => {
        const errors = [];
        const elements = document.querySelectorAll('button, input[type="submit"], input[type="button"], [role="button"], a');
        for (const el of elements) {
          const style = window.getComputedStyle(el);
          const bgColor = style.backgroundColor;
